...
2. **Connect Repository** (choose one):
   ...
   - **Option C**: Manual Deploy (easiest for testing):
     - Click "Manual Deploy"
     - Upload files: `proxy-server.js` and `proxy-package.json` (rename to `package.json`)
...
5. Rename `proxy-package.json` to `package.json`
6. Add environment variable: `API_TOKEN`
7. Railway auto-generates domain and static IP
8. Get IP from Railway dashboard or logs
9. Whitelist IP in Clash Royale portal
10. Set `PROXY_URL` in Vercel
